...
def get_program_info(pid):
    ...
    try:
        ...
        program_info['user'] = pwd.getpwuid(uid).pw_name
    except FileNotFoundError:
...
def get_network_info():
    ...
    connections = []
    try:
        output = subprocess.check_output(['ss', '-p', '--tcp', 'state', 'established', 'state', 'listening'])
        lines = output.decode().split('\n')[1:-1]
        for line in lines:
            parts = line.split()
            local_address = parts[3].split(':')[0]
            local_port = parts[3].split(':')[1]
            remote_address = parts[4].split(':')[0]
            remote_port = parts[4].split(':')[1]
            (pgm, pid_x, _) = parts[5][8:-2].split(',')
            pid = int(pid_x[4:-1])
            state = parts[0]
            conn = {
                'lport': local_port,
            ...
            if pid not in programs:
                programs[pid] = {'pid': pid, 'name': pgm, 'LISTEN': [], 'ESTAB': [] }
            if state == 'LISTEN':
                programs[pid]['LISTEN'].append(local_port)
            else:
                programs[pid]['ESTAB'].append(conn)
...
def print_network_info(network_info):
    ...
    #     print(
    #         f"UID: {program_info['uid']}, User: {program_info['user']}, CWD: {program_info['cwd']}, Command: {program_info['command']}")
    ...
    #             print(
    #                 f"lport: {connection['lport']}  rhost: {connection['rhost']}  rport: {connection['rport']}  {connection['status']}")
    #     print()
